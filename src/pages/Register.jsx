...
function Register() {
  ...
  // let cities  = []
  // useEffect(()=>{
  //   axios.get('http://localhost:3000/api/fire')
  //   .then(response => {
  ...
  useEffect(() => {
    ...
    markercoord.forEach(marker => {
      ...
      new mapboxgl.Marker({ color: 'red', })
        .setLngLat(marker)
        .setPopup(new mapboxgl.Popup().setHTML("<h1>Parking Area</h1>"))
        .addTo(map1);
      // }
    })
  ...
  useEffect(() => {
    if (markerRef.current) {
      markerRef.current.setLngLat([realtime[0].location.coords.longitude, realtime[0].location.coords.latitude])
    ...
    async function fetchData() {
      ...
      for (let i = 0; i < cities.length; i++) {
        ...
        const point = {
          latitude: realtime[0].location.coords.latitude,
          longitude: realtime[0].location.coords.longitude
        }
        // console.log(cities[i].parkcoord)

        const subarrays = Object.values(cities[i].parkcoord);
        ...
        // console.log("Parkname",cities[0].parkname);
        setTimeout(() => {
          if (isInAnyPolygon) {
            // realtime[0].location.parkname = cities[i].parkname;
            console.log("Vehicle in this parking area", cities[i].parkname);


            const dbRef = ref(database, 'users/user/' + "kp_27c0e4a224704945bca8b6064a762efc" + '/parkname');

            // Get the current value of parkname from the database
            get(dbRef).then((snapshot) => {
              if (snapshot.exists()) {
                const currentParkname = snapshot.val();

                if (currentParkname !== cities[i].parkname) {
                  update(dbRef, { parkname: cities[i].parkname });
                  console.log("updated parkname");
                }
              } else {
            ...
            // ref(database, 'users/user/' + realtime[1].id + '/location').update({parkname: cities[i].parkname});
          } else {
            console.log("Vehicle not in this parking area", cities[i].parkname);
            // const dbRef = ref(database, 'users/user/' + "kp_27c0e4a224704945bca8b6064a762efc" + '/parkname');
            // get(dbRef).then((snapshot) => {
            //   if (snapshot.exists()) {
            //     const currentParkname = snapshot.val();
            //     console.log("Current Parkname", currentParkname);
            //     update(dbRef, { parkname: "" });
            //   }
            // }).catch((error) => {
            //   console.error(error);
            // });
